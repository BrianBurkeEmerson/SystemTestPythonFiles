...
global startTime
startTime = datetime.now()
DeviceCount = 3                                  #Number of devices on network... 1 for single unit join test
def main():
    gw_name = 'brianGW'
    username = 'root'
    password = 'emerson1'
...
#
    ...
    while done[0] == 'no' or thread.is_alive():
        ...
        elif done[1] == 'waiting on thread':
            print("waiting on HartServer.txt monitoring thread to exit")
            time.sleep(3)
            continue
        for i in DevList:
            i.ShowData()
        if len(DoneList) >= DeviceCount + 1:
            done[0] = 'yes'
            done[1] = 'waiting on thread'
    Macfinder(chan,done)
